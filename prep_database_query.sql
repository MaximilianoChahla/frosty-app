...
CREATE VIEW IF NOT EXISTS FROSTY_SAMPLE.CYBERSYN_FINANCIAL.FINANCIAL_ENTITY_ATTRIBUTES_LIMITED
AS
SELECT *
FROM financial__economic_essentials.cybersyn.financial_institution_attributes
WHERE VARIABLE IN (
    'ASSET',
    'ESTINS',
    'LNRE',
...
-- Confirm the view was created correctly - should show 5 rows with variable name and definition
SELECT *
FROM FROSTY_SAMPLE.CYBERSYN_FINANCIAL.FINANCIAL_ENTITY_ATTRIBUTES_LIMITED
;

-- Create the modified time series view
CREATE VIEW IF NOT EXISTS FROSTY_SAMPLE.CYBERSYN_FINANCIAL.FINANCIAL_ENTITY_ANNUAL_TIME_SERIES
AS
SELECT
